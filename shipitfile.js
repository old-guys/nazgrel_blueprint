module.exports = function (shipit) {
  ...
  // auto restart process after deploy published
  shipit.on('published', function () {
    shipit.start('pm2_start');
  });
  ...
  shipit.task('pm2_start', function () {
    ...
    // stop and delete from pm2 process list
    shipit.start('pm2_delete');
    // start and daemonize
    ...
    setTimeout(function () {
      ...
      shipit.remote('cd /ishanggang/apps/nazgrel_blueprint/current && pm2 start npm --name zmall_mobile_mock -- run start_zmall_mobile');
  ...
  shipit.task('pm2_delete', function () {
    // stop and delete from pm2 process list
    shipit.remote("pm2 delete channel_mock web_mock dev_mock open_mobile_mock && sleep 5 && pm2 list");
  });
};
